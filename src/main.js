...
router.isReady().then(() => {
  const publicRoutes = ['/log-in', '/sign-up', '/lost-password', '/reset-password', '/confirm-email', '/'];
  if (!publicRoutes.includes(router.currentRoute.value.path)) {
    ...
    console.log('[MAIN] Skipping session restoration on public route:', router.currentRoute.value.path);
  }
});
